...
<template>
  <div class="flex flex-col w-full h-full justify-between">
    <div class="flex flex-col w-full h-full justify-center items-center gap-2">
      ...
      <p class="font-semibold text-xl text-center">{{ $t('chat.whatToSay') }}</p>
    </div>
    <div v-if="!isMobile" class="w-full flex px-12 pl-16 flex-auto">
      <NList v-for="n in 2" :key="n"
        clickable hoverable :show-divider="false" class="w-1/2">
        <NListItem v-for="(item, index) of promptData.slice((n-1)*2, n*2)" :key="(n-1)*2+index"
          ...
          <EmptyPrompt :title="item.data.title" :index="(n-1)*2+index"
            :content="item.data.content" :isMouseOn="item.isMouseOn" />
        </NListItem>
      </NList>
    </div>
    <div v-else class="w-full flex justify-center px-12 pl-16 flex-auto">
      <NList clickable hoverable :show-divider="false" class="w-full">
        <NListItem class="border rounded-md"
          @mouseover="item.isMouseOn = true"
          @mouseout="item.isMouseOn = false"
          @click="sendRecommendation(item.data.content)"
          @touchend="sendRecommendation(item.data.content)"
          v-for="(item, index) of promptData"
              :key="index">
            <EmptyPrompt :title="item.data.title" :content="item.data.content"
               :isMouseOn="item.isMouseOn" :index="index"/>
        </NListItem>
      </NList>
    </div>
  </div>
