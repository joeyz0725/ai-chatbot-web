...
import EmptyPrompt from '@/components/common/EmptyPrompt/index.vue'
import { useBasicLayout } from '@/hooks/useBasicLayout'
import { t } from '@/locales'
import { useAppStore } from '@/store'

interface Emit {
  (ev: 'selectRecommendedPrompt', prompt: string): void
}
...
const promptData = ref([
  {
    data: {
      title: t('Prompts.Prompt1'),
      content: t('Prompts.content1'),
    },
    isMouseOn: false,
  }, {
    data: {
      title: t('Prompts.Prompt2'),
      content: t('Prompts.content2'),
    },
    isMouseOn: false,
  },
  {
    data: {
      title: t('Prompts.Prompt3'),
      content: t('Prompts.content3'),
    },
    isMouseOn: false,
  }, {
    data: {
      title: t('Prompts.Prompt4'),
      content: t('Prompts.content4'),
    },
    isMouseOn: false,
  }])

const sendRecommendation = (prompt: string) => {
  // emit('update:recommendedPrompt', prompt)
  emit('selectRecommendedPrompt', prompt)
}
</script>
...
<template>
  <div class="flex flex-col w-full h-full justify-between">
    ...
    <div v-if="!isMobile" class="w-full flex px-12 pl-16 flex-auto">
      <NList
        v-for="n in 2" :key="n"
        clickable hoverable :show-divider="false" class="w-1/2"
      >
        <NListItem
          v-for="(item, index) of promptData.slice((n - 1) * 2, n * 2)" :key="(n - 1) * 2 + index"
          class="border rounded-md"
          @mouseover="item.isMouseOn = true"
          @mouseout="item.isMouseOn = false"
          @click="sendRecommendation(item.data.content)"
          @touchend="sendRecommendation(item.data.content)"
        >
          <EmptyPrompt
            :title="item.data.title" :index="(n - 1) * 2 + index"
            :content="item.data.content" :is-mouse-on="item.isMouseOn"
          />
        </NListItem>
      </NList>
    </div>
    <div v-else class="w-full flex justify-center px-12 pl-16 flex-auto">
      <NList clickable hoverable :show-divider="false" class="w-full">
        <NListItem
          v-for="(item, index) of promptData"
          :key="index"
          class="border rounded-md"
          @mouseover="item.isMouseOn = true"
          @mouseout="item.isMouseOn = false"
        ...
        >
          <EmptyPrompt
            ...
            :is-mouse-on="item.isMouseOn" :index="index"
          />
        </NListItem>
      </NList>
    </div>
